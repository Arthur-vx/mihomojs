...
- 延迟选优：在香港、新加坡、日本、台湾、美国节点中自动选择延迟较低的节点，适合作为默认代理入口。
- 故障转移：按香港、新加坡、日本、台湾、美国的顺序优先使用可用节点，适合需要稳定出口、不希望频繁跳节点的服务。
- 负载均衡(散列)：同一目标尽量固定到同一节点，适合下载、游戏更新等需要多节点分摊但保持连接相对稳定的场景。
- 负载均衡(轮询)：按请求轮换节点，适合临时测试或并发下载，不建议用于登录、券商、支付等对出口一致性敏感的服务。
- 以上四组不再使用全部节点，会自动过滤订阅信息类节点，并只从常用地区节点生成候选池。
